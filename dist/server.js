...
const getEmployeesData = () => {
    pool
        .query("SELECT employee.first_name,employee.last_name, role.title, role.salary, department.name, CONCAT(e2.first_name,' ',e2.last_name) as manager FROM employee LEFT JOIN role on employee.role_id = role.id LEFT JOIN department on role.department_id = department.id LEFT JOIN employee e2 on employee.manager_id = e2.id")
        .then(({ rows }) => {
        console.table(rows);
        init();
    });
...
const addRole = () => {
    pool
        ...
        pool
            .query("INSERT INTO role (title, salary, department_id) VALUES ($1, $2, $3)", [answers.roleName, answers.roleSalary, answers.selectDepartment])
            .then(() => init());
    });
};
